...
const Main = () => {
    ...
    return (
        <div className="relative min-h-screen  bg-gray-400 dark:bg-gray-900">
            {/* Mobile Header */}
            ...
            {sidebarVisible && (
                <div className="lg:hidden top-20 fixed inset-0 bg-black/50 z-40" onClick={toggleSidebar} />
            )}

            <div className="flex h-screen pt-16 lg:pt-0">
                {/* Sidebar */}
                <motion.nav
                    className="fixed lg:relative z-40 top-12 lg:top-0 left-0 bg-gray-800 text-white w-64 h-full shadow-xl lg:shadow-none lg:flex"
                    initial={{ x: "-100%" }}  // Start hidden (for mobile)
                    animate={{ x: sidebarVisible || window.innerWidth >= 1024 ? 0 : "-100%" }} // Always visible on desktop
                ...
                {/* Main Content */}
                <main className="flex-1 overflow-y-auto bg-gray-400 dark:bg-gray-900">
